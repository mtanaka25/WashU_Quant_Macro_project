...
def x_shock_simulation(init_dist_H,
    ...
    irf_dist_H = np.zeros((N_a, N_z, N_x, N_x, T+1))
    ...
    for a_idx in range(N_a):
        for z_idx in range(N_z):
            for x_idx in range(N_x):
                ...
                for x2_idx in range(N_x):
                    ave += x_H[a_idx, z_idx, x_idx, x2_idx] * distribution_H[a_idx, z_idx, x_idx, x2_idx]
    return ave
...
def group_mean(x, distribution):
    ...
    if dim == 3:
        ...
        N_a, N_z, N_x, _ = distribution.shape
        for a_idx in range(N_a):
            for z_idx in range(N_z):
                for x_idx in range(N_x):
                    for x2_idx in range(N_x):
                        ave += x[a_idx, z_idx, x_idx, x2_idx] * distribution[a_idx, z_idx, x_idx, x2_idx] / total
    return ave
...
def calc_aggregate_irfs(irf_dist_H,
    ...
    a_arrayH = np.zeros((N_a, N_z, N_x, N_x))
    a_arrayN = np.zeros((N_a, N_z, N_x))
...
def calc_inequality_irfs(irf_dist_H,
    ...
    # Calculation
    for t in range(T):
        gini_a_vec[t] = calc_population_gini(a_arrayH, a_arrayN, irf_dist_H[:, :, :, :, t], irf_dist_N[:, :, :, t])
