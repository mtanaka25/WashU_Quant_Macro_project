...
def get_distribution_under_specific_x(trans_prob_z,
    ...
    pop_H = np.zeros((N_a, N_z, N_x, N_x))
    pop_N = np.zeros((N_a, N_z, N_x))
    pop_H[:, :, fixed_x_idx, fixed_x_idx] = each_density
        ...
        for zp_idx in prange(N_z):
            for a_idx in range(N_a):
                for z_idx in range(N_z):
                    H_pre = pop_H_pre[a_idx, z_idx, fixed_x_idx, fixed_x_idx]
                    N_pre =  pop_N_pre[a_idx, z_idx, fixed_x_idx]
                    Pz = trans_prob_z[z_idx, zp_idx]
                    Pd = Pd = default_prob[a_idx, z_idx, fixed_x_idx, fixed_x_idx]
                    Pp = purchase_prob[a_idx, z_idx, fixed_x_idx]
                    pop_H[a_star_HR_idx[a_idx, z_idx, fixed_x_idx, fixed_x_idx], zp_idx, fixed_x_idx, fixed_x_idx] += (1-Pd) * Pz * H_pre
                    pop_N[a_star_HD_idx[a_idx, z_idx, fixed_x_idx, fixed_x_idx], zp_idx, fixed_x_idx] += Pd * Pz * H_pre
                    pop_H[a_star_NP_idx[a_idx, z_idx, fixed_x_idx], zp_idx, fixed_x_idx, fixed_x_idx] += Pp * Pz * N_pre
...
def get_joint_transition_matrix(trans_prob_z,
    ...
    N_zx = N_x * N_x
    N_zxx = N_x * N_x * N_x
    N_azxx = N_a * N_z * N_x * N_x
    N = (N_x + 1) * N_a * N_z * N_x
    # define a helper function
    def cumulative_idx(h_idx, a_idx, z_idx, x_idx, x2_idx = 0):
        if h_idx == 0:
            return N_zxx * a_idx + N_xx * z_idx + N_x * x_idx + x2_idx
        else:
            return N_azxx + N_zx * a_idx + N_x * z_idx + x_idx
                ...
                for z_idx in range(N_z):
                    for x_idx in range(N_x):
                        ...
                        for x2_idx in range(N_x):
                            a_prime_idx_HH = a_star_HR_idx[a_idx, z_idx, x_idx, x2_idx]
                            a_prime_idx_HN = a_star_HD_idx[a_idx, z_idx, x_idx, x2_idx]
                            row_H = cumulative_idx(0, a_idx, z_idx, x_idx, x2_idx)
                            col_HH = cumulative_idx(0, a_prime_idx_HH, z_prime_idx, x_prime_idx, x2_idx)
                            col_HN = cumulative_idx(1, a_prime_idx_HN, z_prime_idx, x_prime_idx)
                            Pd = default_prob[a_idx, z_idx, x_idx, x2_idx] # Prob of H to N
                        ...
                        col_NH = cumulative_idx(0, a_prime_idx_NH, z_prime_idx, x_prime_idx, x_idx)
...
def convert_flatten_dist_to_array(flatten_distribution, N_a, N_z, N_x):
    ...
    for a_idx in range(N_a):
        for z_idx in range(N_z):
                for x_idx in range(N_x):
                    for x2_idx in range(N_x):
                        idx_H = cumulative_idx(0, a_idx, z_idx, x_idx, x2_idx)
                        pop_H[a_idx, z_idx, x_idx, x2_idx] = flatten_distribution[idx_H]
...
def get_stationary_dist_by_iter(trans_prob_z,
    ...
    N_x = trans_prob_x.shape[0]
    # initialize the distribution (start with uniform)
    each_density = 1/ (N_a * N_z * N_x * (1 + N_x))
    pop_H = np.ones((N_a, N_z, N_x, N_x)) * each_density
                ...
                for a_idx in range(N_a):
                    for z_idx in range(N_z):
                        for x_idx in range(N_x):
                            ...
                            pop_H[a_star_NP_idx[a_idx, z_idx, x_idx], zp_idx, xp_idx, x_idx] += Pp * Pz * Px * N_pre
                            pop_N[a_star_NN_idx[a_idx, z_idx, x_idx], zp_idx, xp_idx] += (1-Pp) * Pz * Px * N_pre
                            for x2_idx in range(N_x):
                                H_pre = pop_H_pre[a_idx, z_idx, x_idx, x2_idx]
                                Pd = default_prob[a_idx, z_idx, x_idx, x2_idx]
                                pop_H[a_star_HR_idx[a_idx, z_idx, x_idx, x2_idx], zp_idx, xp_idx, x2_idx] += (1-Pd) * Pz * Px * H_pre
                                pop_N[a_star_HD_idx[a_idx, z_idx, x_idx, x2_idx], zp_idx, xp_idx] += Pd * Pz * Px * H_pre
        # Convergence check
        diff_pop_H = max(np.abs(pop_H - pop_H_pre).flatten())
        diff_pop_N = max(np.abs(pop_N - pop_N_pre).flatten())
        diff = max([diff_pop_H, diff_pop_N])
