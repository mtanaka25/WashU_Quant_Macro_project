...
def get_distribution_under_specific_x(trans_prob_z,
    ...
    N_x = default_prob.shape[-1] # own x (aggregate x is fixed)
    # initialize the distribution (start with uniform)
    each_density = 1/ (N_a * N_z * (1 + N_x))
    ...
    while (iteration < max_iter) & (diff > tol):
        ...
        H_to_H = (1 - default_prob) * pop_H_pre
        H_to_N = default_prob * pop_H_pre
        for x_idex in prange(N_x):
            H_to_H[:, :, x_idex] = np.ascontiguousarray(H_to_H[:, :, x_idex]) @ trans_prob_z
            H_to_N[:, :, x_idex] = np.ascontiguousarray(H_to_N[:, :, x_idex]) @ trans_prob_z
        ...
        for a_idx in prange(N_a):
            for z_idx in prange(N_z):
                for x_idx in prange(N_x):
                    pop_H[a_star_HR_idx[a_idx, z_idx, x_idx], z_idx, x_idx] += H_to_H[a_idx, z_idx, x_idx]
                    pop_N[a_star_HD_idx[a_idx, z_idx, x_idx], z_idx] += H_to_N[a_idx, z_idx, x_idx]
                pop_H[a_star_NP_idx[a_idx, z_idx], z_idx, fixed_x_idx] += N_to_H[a_idx, z_idx]
                    ...
                    for z_idx in range(N_z):
                        for x_idx in range(N_x):
                            ...
                            for x2_idx in range(N_x):
                                ...
                                pop_H[a_star_HR_idx[a_idx, z_idx, x_idx], zp_idx, xp_idx, x2_idx] += (1-Pd) * Pz * Px * H_pre
