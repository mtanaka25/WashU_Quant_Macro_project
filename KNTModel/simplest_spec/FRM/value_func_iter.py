...
def value_func_iter(z_grid, trans_prob_z, x_grid, trans_prob_x, a_grid,
    ...
    VH_init  = np.zeros((N_a, N_z, N_x, N_x))
    VN_init  = np.zeros((N_a, N_z, N_x))
    rm_init = np.ones((N_a, N_z, N_x, N_x)) * 1.1 * r
    # Prepare arrays to save intermediate values
    V_HR_t, V_HD_t = np.zeros((N_a, N_z, N_x, N_x)), np.zeros((N_a, N_z, N_x, N_x))
    V_NP_t, V_NN_t = np.zeros((N_a, N_z, N_x)), np.zeros((N_a, N_z, N_x))
    # Prepare arrays to save optimal policy
    a_star_HR = np.zeros((N_a, N_z, N_x, N_x), dtype = np.int64)
    a_star_HD = np.zeros((N_a, N_z, N_x, N_x), dtype = np.int64)
    ...
    while (iteration < max_iter) & (diff  > tol):
        ...
        for a_idx, a in enumerate(a_grid):
            for z_idx, z in enumerate(z_grid):
                for x_idx in range(N_x):
                    # Homeowners
                    # If default, individual x (x2 here) does not matter
                    if a_idx <= a_d_max_idx:
                        ...
                        a_star_HD_tmp = a_d_max_idx
                    for x2_idx in range(N_x):
                        V_HR_tmp, a_star_HR_tmp = \
                            V_HR(z = z, a = a, a_prime_vec = a_grid,
                                 h = h_star, ph = ph, rm = rm_t[:, z_idx, x_idx, x2_idx],
                                 r = r, beta = beta, alpha = alpha, sigma = sigma,
                                 gamma = gamma, d = d, trans_prob_z_vec = trans_prob_z[z_idx, :],
                                 trans_prob_x_vec = trans_prob_x[x_idx, :],
                                 V_H_prime = VH_tp1[:, :, :, x2_idx])
                        # Store the values
                        V_HR_t[a_idx, z_idx, x_idx, x2_idx] = V_HR_tmp
                        a_star_HR[a_idx, z_idx, x_idx, x2_idx] = a_star_HR_tmp
                        V_HD_t[a_idx, z_idx, x_idx, x2_idx] = V_HD_tmp
                        a_star_HD[a_idx, z_idx, x_idx, x2_idx] = a_star_HD_tmp
                    # Non-homeowners
        ...
        for a_idx in range(N_a):
            for z_idx in range(N_z):
                for x_idx in range(N_x):
                    for own_x_idx, own_x in enumerate(x_grid):
                        rm_tm1[a_idx, z_idx, x_idx, own_x_idx] = \
